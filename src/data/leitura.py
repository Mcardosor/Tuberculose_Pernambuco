...
def incidencia(esc: Escopo) -> dict:
    ...
    Com `Escopo.municipios` (macrorregião ou região de saúde) lê a partição
    ``MUN`` e **soma** os municípios da lista — é o que faz os cards seguirem
    o clique no mapa. A taxa é recalculada da soma, nunca tirada como média.
    ...
    if esc.municipios:
        sql = f"""
            SELECT sum(casos_total) AS casos_total, sum(casos_cura) AS casos_cura,
                   ...
                   sum(casos_total) / nullif(sum(pop_total), 0) * 100000 AS incid_100k_total
            FROM read_parquet('{fonte}', hive_partitioning=true)
            WHERE {onde}
        """
        return _uma_linha(sql, params)
    # `SELECT *` de propósito: o esquema varia por doença e é uma linha só.
    # Quem consome pega a coluna que existir com `.get`.
    sql = f"""
        SELECT *
        FROM read_parquet('{fonte}', hive_partitioning=true)
    """
...
def incidencia_0_14(esc: Escopo) -> dict:
    ...
    Com `Escopo.municipios`, soma a partição ``MUN`` como :func:`incidencia`.
    """
    particao, onde, params = particao_e_filtro_geo(esc, col_mun="cod_mun6")
    fonte = caminho(
        "incidence_0_14",
    ...
    sql = f"""
        SELECT sum(casos_0_14_total) AS casos_0_14_total,
               sum(casos_0_14_cura) AS casos_0_14_cura,
               sum(pop_0_14_total) AS pop_0_14_total,
               sum(casos_0_14_total) / nullif(sum(pop_0_14_total), 0) * 100000
                   AS incid_0_14_100k_total
        FROM read_parquet('{fonte}', hive_partitioning=true)
    """
    if onde:
        sql += f" WHERE {onde}"
...
def obitos_sim(esc: Escopo) -> float | None:
    ...
    if onde:
        sql += f" WHERE {onde}"
    linha = conectar().execute(sql, list(params)).fetchone()
...
def serie_mensal(esc: Escopo) -> pd.DataFrame:
    ...
    **Agrega por mês**: com `Escopo.municipios` a partição é a ``MUN`` e há
    uma linha por município e mês. A população da região vem do `incidence`,
    não da soma do `_cache_ts` — este só tem linha para município **com caso
    no mês**, e somar `pop_total` dali dava uma população que oscilava
    50 mil habitantes de um mês para o outro no Agreste. A taxa é recalculada
    da soma dos casos sobre essa população fixa.
    """
    ...
    onde = f" WHERE {onde_geo}" if onde_geo else ""
    sql = f"""
        WITH por_mun AS (
            SELECT mes, any_value(mes_nome) AS mes_nome, geo_id,
        ...
        SELECT mes, any_value(mes_nome) AS mes_nome,
               sum(casos) AS casos, sum(casos_obitos) AS casos_obitos,
               sum(casos_cura) AS casos_cura, sum(pop_total) AS pop_total,
               sum(casos) / nullif(sum(pop_total), 0) * 100000 AS incid_100k
        FROM por_mun
        GROUP BY mes ORDER BY mes
    """
    df = conectar().execute(sql, list(params_geo)).fetchdf()
    if esc.municipios and not df.empty:
        pop = incidencia(esc).get("pop_total")
        if pop:
            df["pop_total"] = float(pop)
            df["incid_100k"] = df["casos"] / float(pop) * 100_000
    return df
...
def composicao(
    ...
    if dados.empty:
        return pd.DataFrame(columns=["categoria", "n", "pct", "total"])
    # Códigos distintos com o mesmo rótulo viram **uma** categoria. O
    # `valor_lbl` de `SITUA_ENCE` vem reagrupado — abandono, óbito e falência
    # são todos "Desfavorável" — e sem esta soma o gráfico empilhava três
    # barras no mesmo rótulo, uma por cima da outra.
    dados = (
        dados.groupby("categoria", sort=False, as_index=False)
        .agg(valor=("valor", "first"), n=("n", "sum"))
    )
