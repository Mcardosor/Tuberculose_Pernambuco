...
class Kpis:
    ...
    #: Numerador e denominador das outras duas proporções, pelo mesmo motivo:
    #: a fração sob o card tem de ser a conta que produziu a porcentagem, não
    #: outra leitura dos mesmos dados. `interrupcao_base` não é sempre igual
    #: a `encerramentos` — sob a regra `ms` os não avaliados saem.
    hiv_positivos: float | None = None
    hiv_testados: float | None = None
    interrupcoes: float | None = None
    interrupcao_base: float | None = None
...
def calcular(esc: Escopo, regra_interrupcao: str | None = None) -> Kpis:
    ...
    Serve também a macrorregião e a região de saúde: com `Escopo.municipios`
    preenchido, cada leitor soma a partição ``MUN`` dos municípios da lista,
    e os seis cards saem da mesma conta. É o que faz a faixa inteira seguir
    o clique no mapa.
    """
    ...
    hiv = hiv_contagem(esc)
    inter = interrupcao_contagem(esc, regra_interrupcao, enc)
    ...
    return Kpis(
        ...
        hiv_pos_pct=_div(hiv[0], hiv[1], 100),
        hiv_positivos=hiv[0],
        hiv_testados=hiv[1],
        interrupcao_trat_pct=_div(inter[0], inter[1], 100),
        interrupcoes=inter[0],
        interrupcao_base=inter[1],
    )
...
def hiv_contagem(esc: Escopo) -> tuple[float | None, float | None]:
    """(positivos, testados) do HIV na tuberculose; ``(None, None)`` fora dela.
...
def interrupcao_contagem(
    esc: Escopo, regra: str | None = None, df=None
) -> tuple[float | None, float | None]:
    """(interrupções, base) a partir de SITUA_ENCE (tuberculose).
