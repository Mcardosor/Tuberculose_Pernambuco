...
def test_mensal_e_anual_divergem_por_criterio_geografico() -> None:
    ...
    # Residência (`incidence`) × notificação (`_cache_ts`): a distância é
    # de poucos por cento, não de dezenas.
    assert abs(mensal - anual) / anual < 0.1, "distância fora do esperado (residência × notificação)"
