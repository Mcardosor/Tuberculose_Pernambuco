...
def test_ano_corrente_e_parcial() -> None:
    ...
    """
    meses = leitura.meses_com_dado(pack.DOENCA, 2025)
    assert 0 < meses < 12


def test_ano_fechado_nao_e_marcado_como_parcial() -> None:
    """Ano parcial se **detecta**, não se presume.

    Marcar ano fechado como parcial desacredita o aviso quando ele for
    verdadeiro: 2024 tem os 12 meses no `_cache_ts` da TB.
    """
    assert leitura.meses_com_dado(pack.DOENCA, 2024) == 12
